...
two_conditions_plot <- function(data, condition_x, condition_y) {
  ...
  data %>%
    mutate(likelyhood = submission_count / nrow(data)) %>%
    ggplot(aes(x = .data[[condition_x]], y = .data[[condition_y]], fill = likelyhood)) %>%
    heat_map_design() +
...
improvement_plot <- function(data, fixed_condition, changing_condition) {
  ...
  likelyhoods <- merge(data, totals, by = changing_condition) %>%
    filter(.data[[fixed_condition]] == "Yes") %>%
    mutate(likelyhood = submission_count / total)

  likelyhood <- function(data, condition) {
    (data %>% filter(.data[[changing_condition]] == condition))$likelyhood[[1]]
  }

  improvement <- ((likelyhoods %>% likelyhood("Yes")) / (likelyhoods %>% likelyhood("No"))) - 1
