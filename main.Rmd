...
bike_theft_counts <- bind_rows(
  data %>% bike_stolen_never_stolen(data$bike_retrived == 'Yes', 'Bike retrieved'),
  data %>% bike_stolen_never_stolen(data$bike_retrived == 'No', 'Bike not retrieved'),
...
Eine Aussage zur Effektivität lässt sich durch das Vergleichen der bedingten Wahrscheinlichkeit treffen.

> Wie effektiv ist die Rahmennummer zur Fahrraddiebstahlaufklärung?
...
two_conditions_plot <- function(data, condition_column_x, condition_column_y) {
  condition_column_x <- deparse(substitute(condition_column_x))
  condition_column_y <- deparse(substitute(condition_column_y))

  data %>%
    filter(!is.na(data[condition_column_x]), !is.na(data[[condition_column_y]])) %>%
    group_by(.data[[condition_column_x]], .data[[condition_column_y]]) %>%
    summarize(likelyhood = n() / nrow(data)) %>%
    ggplot(aes(x = .data[[condition_column_x]], y = .data[[condition_column_y]], fill = likelyhood)) %>%
    heat_map_design() +
    geom_text(aes(label = scales::percent(likelyhood)), color = 'white', size = 4)
}

stolen_bikes %>%
  two_conditions_plot(bike_retrived, frame_number_record) +
  labs(
...
```
Die Wahrscheinlichkeit zum Wiedererhalt des Fahrrads ist mit dem Wissen über die Rahmennummer um 60 % höher als wenn
diese nicht bekannt ist.

> Wie effektiv ist die polizeiliche Meldung eines Fahrraddiebstahls zur Aufklärung?

```{r, fig.width = 11}
stolen_bikes %>%
  two_conditions_plot(bike_retrived, police_report) +
  labs(
...
```
Eine polizeiliche Meldung eines Diebstahls erhöht die Chance zum Wiedererhalten des Fahrrades lediglich um 9 %.


> Wie häufig wurde ein Diebstahl gemeldet in Abhängigkeit vom Wissen der Rahmennummer?
...
Ist die Rahmennummer bekannt, wird ein Diebstahl mit nahezu doppelter Wahrscheinlichkeit polizeilich gemeldet, als wenn
die Rahmennummer nicht bekannt ist.
