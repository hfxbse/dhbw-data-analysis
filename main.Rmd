...
> Was ist die Aufklärungsquote in Abhängigkeit vom Wissen über die Rahmennummer?
...
case_solved_rate_plot <- function(data, condition_column) {
  condition_column <- deparse(substitute(condition_column))

  data %>%
    filter(!is.na(bike_retrived), !is.na(data[[condition_column]])) %>%
    group_by(bike_retrived, .data[[condition_column]]) %>%
    summarize(likelyhood = n() / nrow(stolen_bikes)) %>%
    ggplot(aes(x = bike_retrived, y = .data[[condition_column]], fill = likelyhood)) %>%
    heat_map_design() +
    geom_text(aes(label = scales::percent(likelyhood)), color = 'white', size = 4)
}

stolen_bikes %>%
  case_solved_rate_plot(frame_number_record) +
  labs(
    title = 'Aufklärungsquote der Fahrraddiebstähle in Abhängikeit zur Rahmennummer',
...
> Was ist die Aufklärungsquote in Abhängigkeit vom Involvement der polizeilichen Behörden?

```{r, fig.width = 11}
stolen_bikes %>%
  case_solved_rate_plot(police_report) +
  labs(
    title = 'Aufklärungsquote der Fahrraddiebstähle in Abhängikeit zur polizeilicher Behörde',
    x = 'Fahrrad zurückerhalten',
