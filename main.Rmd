...
retrieved_bike_counts <- data %>%
  stolen_never_stolen(
    stated_condition = data$bike_retrived == 'Yes',
    frequency = data$bike_stolen_frequency
  )

retrieved_bike_counts %>%
  donut_diagram(
    values = retrieved_bike_counts$submission_count,
    fill = retrieved_bike_counts$frequency,
    annotation = retrieved_bike_counts %>% summarize(submission_count = sum(submission_count))
  ) +
  labs(
...
> Wie viele Fahrräder sind zu ihrem Besitzer nie zurückgekommen, und wie viele davon wurden davor nie gestohlen?

```{r, fig.width = 10}
never_retrieved_bikes <- data %>%
  stolen_never_stolen(
    stated_condition = data$bike_retrived == 'No',
    frequency = data$bike_stolen_frequency
  )

never_retrieved_bikes %>%
  donut_diagram(
    values = never_retrieved_bikes$submission_count,
    fill = never_retrieved_bikes$frequency,
    annotation = never_retrieved_bikes %>% summarize(submission_count = sum(submission_count))
  ) +
  labs(
    title = 'Anzahl der nie zurückerhaltenen Fahrräder',
    fill = 'Fahrrad gestohlen'
  )
```
...
bind_rows(
  ...
  geom_text(aes(label = scales::percent(theft_frequency)), vjust = -1, colour = "black") +
  theme(
    legend.position = 'none',
    axis.ticks = element_blank(),
